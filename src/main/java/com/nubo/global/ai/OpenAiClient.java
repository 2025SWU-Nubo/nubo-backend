...
public class OpenAiClient {
  ...
  private String buildPrompt(String inputText) {
    return String.format("""
      ...
      - transcript나 subtitle 내에 언어가 혼용되어 있거나 발음 인식 오류가 있는 경우,
        영상의 주제나 의도를 파악하기 위한 **자연스러운 수준의 유추**는 허용한다.
        (예: 영어 단어 발음 설명 영상에서 단어가 잘못 인식된 경우, 해당 단어를 복원하거나 올바른 형태로 표현 가능)
      - 단, 의미를 왜곡하거나 새로운 정보를 추가하는 추측은 여전히 금지한다.
      ...
      - Unordered list: `-` 사용, 각 하위 리스트는 레벨당 공백 4칸 들여쓰기로 작성한다.
        - 단, **Unordered list 하위에는 Ordered list(`1.`, `2.` 등)를 절대 포함하지 않는다.**
      - Ordered list: 항상 `1.` 으로 표기하며, 각 하위 리스트는 레벨당 공백 4칸 들여쓰기로 작성한다.
      - Bold: `**굵게**` 표현 허용
      - 문단(heading/문장) 사이에는 빈 줄 한 줄(`\\n\\n`)을 넣어 구분한다.
        - 단, **Ordered list(`1.`, `2.` 등) 항목들 사이에는 빈 줄을 넣지 않는다.**
      ...
      - 학습 노트 스타일의 문장을 명사형 종결 어미 위주로 작성한다.
      - 광고, 홍보, 과장, 클릭 유도 금지. 사실 서술 위주로 작성한다.
      - 문단 간에는 반드시 빈 줄(`\\n\\n`)을 삽입한다. 단, 리스트 사이에는 삽입하지 않는다.
  ...
  public Map<String, Object> regenerateSummary(
    ...
    String userPrompt = String.format("""
        ...
        - Unordered list: `-` 사용, 각 하위 리스트는 레벨당 공백 4칸 들여쓰기로 작성한다.
          - 단, **Unordered list 하위에는 Ordered list(`1.`, `2.` 등)를 절대 포함하지 않는다.**
        - Ordered list: 실제 순서를 반영해 `1.`, `2.`, `3.` ... 형태로 작성하며,
          각 하위 리스트는 레벨당 공백 4칸 들여쓰기로 작성한다.
        - Bold: `**굵게**` 표현 허용
        - 문단(heading/문장) 사이에는 빈 줄 한 줄(`\\n\\n`)을 넣어 구분한다.
          - 단, **Ordered list(`1.`, `2.` 등) 항목들 사이에는 빈 줄을 넣지 않는다.**
